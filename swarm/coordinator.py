"""
Swarm Coordinator — Agent Lifecycle Management for the Hayek Economy

Adapted from NecroSwarm's SwarmCoordinator. Simplified to focus on
agent lifecycle management with Docker sandbox integration.

Provides swarm-native coordination capabilities including:
...
class SwarmCoordinator:
    ...
    async def _handle_message(self, message: SwarmMessage) -> None:
        ...
        if message.message_type == "consensus_vote":
            ...
            # Forward to consensus engine (handled externally by HayekMAS)
            logger.info(f"Consensus vote from {vote.voter_id}: {vote.choice}")
